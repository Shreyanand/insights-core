...
def test_get_cib():
    cib = CIB(context_wrap(CIB_XML, path='/var/lib/pacemaker/cib/cib.xml'))

    assert cib is not None
    assert cib.dom is not None
    opts = cib.dom.find(".//cluster_property_set[@id='cib-bootstrap-options']")
    assert opts is not None
    print opts.get('id')
    assert opts.get('id') == 'cib-bootstrap-options'

    assert sorted(cib.nodes) == sorted(['odchw1', 'pyatt', 'ford'])
